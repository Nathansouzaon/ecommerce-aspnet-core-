...
namespace App.Pagamentos.API.Services
{
    public class PagamentoIntegrationHandler : BackgroundService
    {
        ...
        private async Task CancelarPagamento(PedidoCanceladoIntegrationEvent message)
        {
            ...
            if (response.ValidationResult.IsValid is false) throw new DomainException($"Falha ao cancelar pagamento do pedido {message.PedidoId}");
        }
        ...
        private async Task CapturarPagamento(PedidoBaixadoEstoqueIntegrationEvent message)
        {
            ...
            if (response.ValidationResult.IsValid is false) throw new DomainException($"Falha ao capturar pagamento do pedido {message.PedidoId}");
